...
const DataTable = ({ columns, data, renderRow, currentPage, totalPages, itemsPerPage, onPageChange, totalItems }: any) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 bg-[var(--bg-secondary)] border border-[var(--border-secondary)] rounded-[12px] flex flex-col">
...
const inferAuthMethodFromEvent = (eventType?: string) => {
...
const resolveAuthType = (log: any, richData: any) => {
  const candidates = [
...
const getAmountClass = (level: string, score: number) => {
...
const getTimelineDotColor = (label: string, fallbackResult: string) => {
  const normalized = label.toLowerCase();
  console.log({ normalized, fallbackResult })
  if (
...
const generateTimeline = (e: AuthEvent) => {
  ...
  if (Array.isArray(e.risk.timeline) && e.risk.timeline.length > 0) {
    ...
    return e.risk.timeline.map((step, index) => ({
      label: step,
      time: new Date(t.getTime() - ((totalSteps - 1 - index) * 1000)).toLocaleTimeString('en-US', { hour12: false }),
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      const mappedData: AuthEvent[] = fltrdLogs.map((log: any) => {
        ...
        const deviceOs = deviceInfo.os || [telemetry.os_name, telemetry.os_version].filter(Boolean).join(' ') || log.userAgent || 'Unknown';
        ...
        return {
          ...
          risk: {
            ...
            timeline: Array.isArray(richData.timeline)
              ? richData.timeline.filter((s: any) => typeof s === 'string' && s.trim() !== '')
              : [],
              ...
              renderRow={(e: AuthEvent) => (
                <tr key={e.id} onClick={() => setSelectedEvent(e)} className="group border-b border-[var(--border-secondary)] hover:bg-[var(--bg-hover)] cursor-pointer transition-colors">
                  <td className="p-3 pl-5">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-[20px] text-xs font-medium 
                    ${e.resultLabel === 'Success' && ['logged in', 'registered'].some(word => e.type.toLowerCase().includes(word)) ? 'bg-[var(--info-bg)] text-[var(--info)]' : e.resultLabel === 'Success' ? 'bg-[var(--success-bg)] text-[var(--success)]' :
                  ...
                  <td className="p-3">
                    <div className="flex flex-wrap gap-1.5 max-w-[220px]">
                      {buildRiskContextTags({
                        ...
                        factors: [
                          ...e.risk.tags,
                          ...e.risk.reasons.map((r) => ({ label: r.label, rule: r.rule })),
                        ],
                        stepUpRequired: e.risk.stepUpRequired,
                        extraLabels: [e.risk.network || '', e.risk.beneficiary || ''],
                      }).map((tag, i) => {
                       if (tag.label) return <span
                          key={`${e.id}-risk-${i}-${tag.label}`}
        ...
        <SlideOver
          ...
          footer={
            <div className="flex gap-2 w-full">
              <button onClick={() => router.push(`/users?query=${encodeURIComponent(selectedEvent?.userId || '')}`)} className="flex-1 py-2 border border-[var(--border-primary)] rounded-[6px] text-[13px] text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)] transition-all">
                <div className="flex items-center justify-center gap-2">
                  <User className='w-3.5 h-3.5' />
                  <p>View User</p>
                </div>
              </button>
              <button onClick={() => router.push(`/devices?query=${encodeURIComponent(selectedEvent?.userId || '')}`)} className="flex-1 py-2 border border-[var(--border-primary)] rounded-[6px] text-[13px] text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)] transition-all">
                <div className="flex items-center justify-center gap-2">
          ...
          {selectedEvent && (
            <div className="space-y-6">
              ...
              <div className="pb-2">
                ...
                {selectedEvent.risk.amount && <DetailRow label="Transaction Amount" value={selectedEvent.risk.amount} valueClass={`amount-badge ${selectedEvent.risk.amountClass || 'low'}`} />}
                <DetailRow label="Beneficiary" value={selectedEvent.risk.beneficiary || '—'} valueClass="font-mono text-[12px]" />
                {selectedEvent.risk.stepUpRequired !== undefined && <DetailRow label="Step Up Required" value={selectedEvent.risk.stepUpRequired ? 'Yes' : 'No'} valueClass={`stepup-badge ${selectedEvent.risk.stepUpRequired ? 'required' : 'not-required'}`} />}
                <div className="flex justify-between py-2.5 border-b border-[var(--border-secondary)]">
                  <span className="text-[13px] text-[var(--text-tertiary)]">Risk Score</span>
                  <span className={`text-[13px] font-bold ${selectedEvent.risk.level === 'LOW' ? 'text-var(--success)' : selectedEvent.risk.level === 'MEDIUM' ? 'text-[var(--warning)]' : 'text-[var(--error)]'}`}>
                ...
                <div className={`${selectedEvent.risk.reasons.length ? 'block' : 'flex justify-between'} py-2.5 border-b border-[var(--border-secondary)]`}>
                  ...
                  {selectedEvent.risk.reasons.length ?
                    <ul className="space-y-2 pl-2 py-1">
                      {selectedEvent.risk.reasons.map((r) => (
                        <li key={r.rule} className="flex justify-between items-start text-[12px]">
                          <span className="text-[var(--text-secondary)]">{r.label} {r.desc && <span className="text-[var(--text-tertiary)]">- {r.desc}</span>}</span>
...
function EventIcon({ type }: { type: string }) {
  if (['passkey', 'login', 'pin'].some(word => type.toLocaleLowerCase().includes(word))) {
    return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
			<title>title</title>
      <rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" />
    </svg>
  }
  if (['approval', 'requested'].some(word => type.toLocaleLowerCase().includes(word))) {
    return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
			<title>title</title>
  ...
  if (['denied', 'blocked'].some(word => type.toLocaleLowerCase().includes(word))) {
    return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
			<title>title</title>
      <circle cx="12" cy="12" r="10" /><line x1="15" y1="9" x2="9" y2="15" /><line x1="9" y1="9" x2="15" y2="15" />
    </svg>
  }
  if (['device', 'revoked'].some(word => type.toLocaleLowerCase().includes(word))) {
    return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
			<title>title</title>
      <circle cx="12" cy="12" r="10" /><line x1="15" y1="9" x2="9" y2="15" /><line x1="9" y1="9" x2="15" y2="15" />
    </svg>
  }
  if (['payment', 'timeout'].some(word => type.toLocaleLowerCase().includes(word))) {
    return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
			<title>title</title>
