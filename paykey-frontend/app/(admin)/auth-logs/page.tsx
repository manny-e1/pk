	...
	return (
		<div className="fixed inset-0 z-50 flex justify-end">
			...
			<div className="relative w-[400px] bg-[var(--bg-secondary)] border-l border-[var(--border-secondary)] h-full shadow-2xl flex flex-col animate-[slideInRight_0.3s]">
				...
				{footer && (
					<div className="p-6 border-t border-[var(--border-secondary)] bg-[var(--bg-tertiary)] shrink-0">
						{footer}
					</div>
				)}
			</div>
				...
				<div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-secondary)] shrink-0">
					<div>
						...
						<span className="text-[12px] text-[var(--text-tertiary)] ml-2">
							Showing {start}-{end} of {totalItems.toLocaleString()} events
						</span>
					</div>
				</div>
						...
						{getPageNumbers().map((pageNum, idx) =>
							pageNum === "..." ? (
								...
								<button
									...
									className={`w-8 h-8 flex items-center justify-center rounded-[6px] text-[13px] font-medium transition-all ${
										currentPage === pageNum
...
const formatType = (t: string) =>
	t ? t.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()) : "Unknown";
...
const generateTimeline = (e: AuthEvent) => {
	...
	else if (e.result.includes("CHALLENGE") || e.result === "TIMEOUT")
	...
	if (e.risk.tags && e.risk.tags.length > 0) {
		e.risk.tags.forEach((tag, i) => {
			timeline.push({
				...
				time: new Date(t.getTime() - (3000 + i * 500)).toLocaleTimeString(
					"en-US",
					{ hour12: false },
				),
...
export default function AuthLogsPage() {
	...
	const fetchLogs = async () => {
		...
		try {
			const rawLogs = await adminService.getAuthLogs(
				eventType || undefined,
				transactionIdFilter || undefined,
			);
			const fltrdLogs = rawLogs.filter(
				(log: any) =>
					!(
						log.user?.role === "ADMIN" &&
						(log.eventType.toLowerCase().includes("passkey") ||
							log.eventType.toLowerCase().includes("registration") ||
			...
			const mappedData: AuthEvent[] = fltrdLogs.map((log: any) => {
				...
				if (Array.isArray(richData.tags)) {
					...
					tagsArray = richData.map((t: any) =>
						typeof t === "string" ? { label: t, class: "default" } : t,
					);
				}
				...
				if (
					...
					statusLabel = "Blocked";
				else if (statusRaw.includes("CHALLENGE") || statusRaw === "TIMEOUT")
				...
				return {
					...
					isNewDevice: tagsArray.some((t) =>
						t.label.toLowerCase().includes("new device"),
					),
					authType: log.authMethod || "Unknown",
					firstSeen: log.user?.keys?.[0]?.registeredTimestamp
						? new Date(log.user.keys[0].registeredTimestamp).toLocaleDateString(
								"en-US",
						...
						model: (() => {
							if (
								(telemetry.device_model &&
									telemetry.device_model.includes("unknown")) ||
								(log.device && log.device.includes("unknown"))
							) {
					...
					location: {
						...
						flag:
							(network.country || log.countryCode) === "MY"
								? "🇲🇾"
								: (network.country || log.countryCode) === "ID"
									? "🇮🇩"
									: (network.country || log.countryCode) === "UK" ||
											(network.country || log.countryCode) === "GB"
										? "🇬🇧"
										: "🌐",
					...
					deviceStatus:
						log.user?.keys.length > 0 ? log.user.keys[0].status : "—",
					risk: {
						score: richData.riskScore || log.riskScore || 0,
						level: richData.riskLevel || "UNKNOWN",
						amount: richData.amount
							? `${richData.currency || "$"}${richData.amount}`
							: undefined,
						amountClass: richData.amount > 1000 ? "high" : "low",
						network: telemetry.is_vpn_active
							? "vpn"
							: richData.isVpn
								? "vpn"
								: undefined,
	...
	const calculateStats = (data: AuthEvent[]) => {
		...
		const todayLogs = data.filter(
			(l) => new Date(l.timestamp).getTime() >= startToday,
		);
		...
		const formatChangeText = (pct: number) =>
			`${pct > 0 ? "↑" : pct < 0 ? "↓" : ""} ${pct > 0 ? "+" : ""}${pct.toFixed(1)}% from yesterday`;
		...
		const successToday = todayLogs.filter(
			(l) => l.result === "SUCCESS" || l.result === "APPROVED",
		).length;
		const rateToday = totalToday ? (successToday / totalToday) * 100 : 0;
		const rateDiff =
			rateToday -
			(totalYest
				? (yesterdayLogs.filter((l) => l.result === "SUCCESS").length /
						totalYest) *
					100
				: 0);

		const failToday = todayLogs.filter(
			(l) =>
				l.result === "BLOCKED" ||
				l.result === "FAILED" ||
				l.result === "REJECTED",
		).length;
		const failPct = getChange(
			failToday,
			yesterdayLogs.filter(
				(l) => l.result === "BLOCKED" || l.result === "FAILED",
			).length,
		);

		const riskToday = todayLogs.filter(
			(l) => l.risk.amountClass === "high" || l.risk.tags?.length > 0,
		).length;
		const riskPct = getChange(
			riskToday,
			yesterdayLogs.filter(
				(l) => l.risk.amountClass === "high" || l.risk.tags?.length > 0,
			).length,
		...
		const pkToday = todayLogs.filter((l) =>
			l.type.toLowerCase().includes("passkey"),
		).length;
		const pkPct = getChange(
			pkToday,
			yesterdayLogs.filter((l) => l.type.toLowerCase().includes("passkey"))
				.length,
		);
	...
	const filteredData = logs.filter((e) => {
		...
		const matchResult =
			resultFilter === "" ||
			e.resultLabel.toLowerCase() === resultFilter.toLowerCase();
		let matchDate = true;
		...
		if (
			activeChips.has("high-risk") &&
			e.risk.amountClass !== "high" &&
			!e.risk.tags?.some(
				(t) => t.class === "high" || t.label.toLowerCase().includes("high"),
			)
		)
			matchChips = false;
			...
			<main className="flex-1 flex flex-col h-screen overflow-hidden">
				<header className="px-6 py-3 border-b border-[var(--border-secondary)] bg-[var(--bg-secondary)] flex justify-between items-center shrink-0 h-[60px]">
					...
					<div className="flex items-center gap-3">
						<div className="flex items-center gap-2 text-xs text-[var(--success)] mr-2">
						...
						<div className="flex items-center gap-2 bg-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-[var(--radius-md)] px-3 py-2 min-w-[280px]">
							...
							/>
							<span className="text-[10px] text-[var(--text-tertiary)] bg-[var(--bg-secondary)] px-1.5 py-0.5 rounded border border-[var(--border-secondary)] font-mono">
								⌘K
							</span>
						</div>
				...
				<div className="flex-1 overflow-auto p-6 flex flex-col">
					...
					<div className="flex items-center gap-3 mb-5 shrink-0 flex-wrap">
						...
						<div className="flex items-center gap-2">
							...
							<div className="flex items-center gap-2 bg-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-[var(--radius-md)] px-3 py-1.5">
								<input
									type="date"
					...
					{loading ? (
						<div className="text-[var(--text-tertiary)] text-center py-10 text-sm">
							Loading logs...
						</div>
					) : (
						<DataTable
							...
							renderRow={(e: AuthEvent) => (
								...
								>
									...
									<td className="p-3">
										<div className="flex items-center gap-2">
											<div className="w-7 h-7 rounded-[var(--radius-md)] bg-[var(--bg-tertiary)] flex items-center justify-center shrink-0">
												...
												>
													{e.device.type.toLowerCase().includes("mobile") ||
													e.device.type.toLowerCase().includes("phone") ? (
														<>
															<rect
																x="5"
																y="2"
											...
											<div className="flex flex-col">
												...
												<div className="flex items-center gap-1.5">
													...
													{e.isNewDevice && (
														<span className="text-[9px] bg-[var(--purple-bg)] text-[var(--purple)] px-1.5 py-0.5 rounded font-bold uppercase tracking-wide">
															New
														</span>
													)}
												</div>
									...
									<td className="p-3">
										<div className="flex items-center gap-2">
											<span className="text-base">
												{countryCodeToEmoji(e.location.country)}
											</span>
											<div className="flex flex-col">
												<span className="text-[13px] text-[var(--text-primary)]">
													{getCountryName(e.location.country)}
												</span>
												<span className="text-[11px] text-[var(--text-tertiary)] font-mono">
													{e.location.ip}
												</span>
									...
									<td className="p-3 text-right">
										<span
											className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-[var(--radius-sm)] text-[12px] font-medium capitalize ${
												e.resultLabel === "Success"
													? "bg-[var(--success-bg)] text-[var(--success)]"
													: e.resultLabel === "Blocked"
														? "bg-[var(--error-bg)] text-[var(--error)]"
				...
				<SlideOver
					...
					footer={
						<div className="flex gap-2 w-full">
							<button
								onClick={() =>
									router.push(`/users?userId=${selectedEvent?.userId}`)
								}
							...
							<button
								onClick={() =>
									router.push(`/devices?email=${selectedEvent?.email}`)
								}
								className="flex-1 py-2 border border-[var(--border-primary)] rounded-[6px] text-[13px] text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)] transition-all"
							>
								<div className="flex items-center justify-center gap-2">
						...
						<div className="space-y-6">
							...
							<div className="pb-2">
								...
								{selectedEvent.isNewDevice && (
									<div className="flex justify-between py-2.5 border-b border-[var(--border-secondary)]">
										...
										<span className="text-[10px] bg-[var(--purple-bg)] text-[var(--purple)] px-2 py-0.5 rounded font-bold uppercase">
											NEW DEVICE
										</span>
									</div>
								)}
							...
							<div className="pb-2">
								...
								<div className="flex justify-between py-2.5 border-b border-[var(--border-secondary)]">
									...
									<span className="text-[13px] text-[var(--text-primary)] text-right">
										<span className="mr-1.5">
											{countryCodeToEmoji(selectedEvent.location.country)}
										</span>
								...
								<DetailRow
									...
									value={
										selectedEvent.location.lat
											? `${selectedEvent.location.lat}, ${selectedEvent.location.long}`
											: "—"
									}
								...
								<div className="flex justify-between py-2.5 border-b border-[var(--border-secondary)]">
									...
									>
										{selectedEvent.risk.network
											? selectedEvent.risk.network.toUpperCase() + " DETECTED"
											: "Clean (No VPN/Proxy)"}
									</span>
								</div>
							</div>
							...
							<div className="pb-2">
								...
								{selectedEvent.risk.amount && (
									<DetailRow
										label="Transaction Amount"
										value={selectedEvent.risk.amount}
										valueClass="amount-badge medium"
									/>
								)}
								<DetailRow
									label="Beneficiary"
								...
								>
									<span className="block text-[11px] text-[var(--text-tertiary)] mb-2 uppercase font-semibold">
										Reason Codes
									</span>
