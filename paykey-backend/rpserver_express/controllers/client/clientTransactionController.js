...
exports.initiateTransaction = async (req, res) => {
	...
	try {
		...
		// Local/demo-only bypass for transaction risk blocking.
		// Keeps production behavior unchanged.
		const riskBypassEnabled =
			process.env.NODE_ENV !== "production" &&
			process.env.DEMO_BYPASS_RISK_BLOCK === "true";

		if (riskBypassEnabled) {
			riskResult.score = Math.min(riskResult.score || 0, 25);
			riskResult.level = "LOW";
			riskResult.isBlockedByAmount = false;
			riskResult.amountMandatedMethods = [];
			riskResult.factors = Array.isArray(riskResult.factors)
				? riskResult.factors.filter((f) => {
						if (typeof f === "string") {
							return !f.toLowerCase().includes("block");
						}
		...
		if (policyDecision.status === "CHALLENGED") {
			...
			return res.json({
				...
				amountMandatedMethods: riskResult.amountMandatedMethods || [],
				policySettings: {
					userVerification:
